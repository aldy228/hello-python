"""
Main Game Module - Snake (Enhanced Version)
...
Foods have different values and disappear after some time.
"""
...
SCREEN_WIDTH = 800   # Was 600, now larger
SCREEN_HEIGHT = 800  # Was 600, now larger
...
def show_ui():
    """
    Display score, level, and food value in top corners of screen
    ...
    # Render food value
    food_value_text = font.render(f"Food: {food.value}pts", True, food.color)
    # Draw below score
    screen.blit(food_value_text, (15, 55))
...
while running:
    ...
    2. Update game logic (movement, collisions, levels, food timer)
    ...
    if not game_over:
        ...
        if snake.get_head() == food.get_position():
            ...
            # Add food value to score (different foods have different values!)
            score += food.value
        ...
        # Draw snake and food
        snake.draw(screen)
        food.draw(screen)  # Food now draws with timer indicator
        
        # Draw UI (score, level, food value)
        show_ui()
    else:
